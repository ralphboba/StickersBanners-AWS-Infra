...
## ★ 세금 (Kai 확정, 2026-09-16)

> "세금 포함 금액으로 보여주는 걸로 해야지. 추가 요금 붙는거 + 거기에 붙는 택스도 또 포함.
> 그리고 새로운 최종 가격을 넣어서 오더데스크 및 쇼피파이 인보이스로 또 출력."

### 원칙 — 세금은 우리가 계산하지 않는다

배송비 과세 여부와 세율은 **배송지에 따라 다르다.** 우리가 만든 숫자는 실제로 빠져나간
금액과 어긋날 수 있고, 그러면 **화면에 $33.96, 카드에서 $36.68**이 된다. 이 기능이 없애려던
바로 그 전화가 온다.
...
**청구될 금액이 아니면 표시하지 않는다.** Shopify가 아직 답하지 않았으면
(`final: false`) 금액 자리에 `—`를 두고 버튼을 잠근다. 세전 금액을 슬쩍 보여주지 않는다.

API 응답:

```json
"upgrade": { "to": "FedEx 1-Day", "shipping": 33.96, "tax": 2.72,
             "total": 36.68, "final": true }
```

화면에는 `total`만 뜬다. 옆에 "includes tax".

### 오더데스크에 쓸 때 — 세 숫자가 세 필드로 간다

한 필드에 몰아넣으면 **총액이 안 맞는다.**

| 필드 | 더하는 값 | 예시 |
| --- | --- | --- |
| `shipping_total` | 배송 차액만 | 128.11 → **162.07** |
| `tax_total` | 세금만 | 0.00 → **2.72** |
| `order_total` | **둘 다** (= 고객이 낸 돈) | 278.11 → **314.79** |

테스트가 쓰기 후에도 `제품 + 배송 + 세금 = 총액`이 성립하는지 검증한다.

노트에도 내역이 남는다:

```
Shipping upgraded FedEx 2-Days -> FedEx 1-Day by customer,
+$33.96 + $2.72 tax = $36.68 (D170)
```

### 결제 후에는 견적이 아니라 실제 금액을 쓴다
