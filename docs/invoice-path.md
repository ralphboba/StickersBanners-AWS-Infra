...
돈은 Shopify(D###)에서만 움직인다. 오더데스크에서 바꾸는 것은 **`shipping_method` 한
필드뿐**이다. 그래서 오더데스크의 주문 총액은 결제 전 금액($278.11)으로 남는다.

> **Kai 확인:** 오더데스크 총액도 맞춰야 하나? ShipStation은 서비스만 보고 금액은 안 보며,
> 회계 진실은 Shopify다. 다만 누군가 오더데스크 기준으로 정산을 뽑는다면 어긋난다.
